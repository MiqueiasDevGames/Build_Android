...
#PAckage Projeto
echo ""
MyPackage=$(cat My_Build/PACKAGE/package.txt)

> tmpp.txt

tr '.' '/' < My_Build/PACKAGE/package.txt > tmpp.txt
pPackage=$(cat  tmpp.txt)
echo $pPackage
rm -rf  tmpp.txt
echo ""
...
#Paga so o necessario
rm -rf My_Build/build/$pPackage/*
...
#APaga so o necessario
 rm -rf My_Build/build/intermediate/dex/$pPackage/*
...
echo "..."
MyClass=$(find  My_Build/build/$pPackage/ -name "*.class" )
...
$realAndroidpath/build-tools/$versionBuildTools/d8 --min-api $minAPI $debugORrelase --lib $androidpath/android.jar  $MyClass --intermediate --file-per-class --output  My_Build/build/intermediate/dex
echo "..."
